...
h1, h2, h3, h4, h5, h6 { font-family: 'Poppins', sans-serif; font-weight: 600; line-height: var(--line-height-tight); letter-spacing: var(--letter-spacing-tight); color: var(--gray-900); }
...
h6 { font-size: var(--font-size-base); font-weight: 600; text-transform: uppercase; }
...
/* ============ PROFESSIONAL DATABASE-STYLE TABLES ============ */
.table-responsive {
    overflow-x: auto;
    border-radius: var(--border-radius-lg);
    background: white;
...
.table tbody tr:hover {
    background: linear-gradient(90deg, rgba(0, 119, 190, 0.04) 0%, rgba(0, 119, 190, 0.08) 100%);
...
.table tbody td:nth-child(1) { min-width: 220px; max-width: 220px; }
.table tbody td:nth-child(2) { min-width: 140px; max-width: 140px; text-align: center; }
.table tbody td:nth-child(3) { min-width: 120px; max-width: 120px; font-family: 'SF Mono', monospace; font-size: var(--font-size-xs); color: var(--gray-700); }
.table tbody td:nth-child(4) { min-width: 160px; max-width: 160px; }
.table tbody td:nth-child(5) { min-width: 180px; max-width: 180px; }
.table tbody td:nth-child(6) { min-width: 120px; max-width: 120px; text-align: center; }
.table tbody td:nth-child(7) { min-width: 100px; max-width: 100px; text-align: center; }
...
/* ============ ENHANCED COMMUNICATIONS STATS TAB ============ */
...
.preview-card.planned {
    border-left-color: var(--medical-blue);
    background: linear-gradient(135deg, rgba(0, 119, 190, 0.05), rgba(0, 119, 190, 0.1));
}
...
.preview-card.active {
    ...
    background: linear-gradient(135deg, rgba(245, 124, 0, 0.05), rgba(245, 124, 0, 0.1));
}
...
@media (max-width: 1024px) {
    .live-stats-sidebar { width: 100%; height: 80vh; top: 20vh; }
...
.empty-state-icon { font-size: 48px; color: var(--gray-400); margin-bottom: 16px; }
.empty-state-title { font-size: 18px; font-weight: 700; color: var(--gray-700); margin-bottom: 8px; }
.empty-state-description { font-size: 14px; color: var(--gray-600); }
...
@media (prefers-color-scheme: dark) {
    ...
    body { background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%); color: #f0f0f0; }
    .card, .modal-container, .table-responsive, .live-stats-sidebar { background: #2d2d2d; border-color: #4d4d4d; color: #f0f0f0; }
    .table thead { background: linear-gradient(135deg, #005a9e, #003f6b); }
    .table tbody tr:nth-child(even) { background-color: #3d3d3d; }
    .sidebar { background: linear-gradient(180deg, #0a2d44 0%, #003f6b 100%); }
...
.skeleton-button { height: 48px; width: 120px; border-radius: var(--border-radius-md); }
.skeleton-card { padding: 24px; border-radius: var(--border-radius-lg); }
...
@media (prefers-reduced-motion: no-preference) {
    .card, .btn, .sidebar-menu-link, .table tbody tr {
...
/* ============ COMPLETE CSS - ALL ORIGINAL CODE + ENHANCEMENTS ============ */
</style>
</head>
<body>
    <div id="app" v-cloak>
        ...
        <div class="modal-body">
            ...
            <div class="space-y-4">
                ...
                <div class="form-group">
                    ...
                    <div class="flex border border-gray-300 rounded-md overflow-hidden">
                        <button type="button" 
                                ...
                                :class="absenceModal.form.leave_type === 'planned' ? 'bg-blue-100 text-blue-700 border-r border-blue-200' : 'bg-gray-50 text-gray-600'"
                                @click="absenceModal.form.leave_type = 'planned'; updatePreview()">
                            <i class="fas fa-calendar-alt mr-2"></i>Planned
                        </button>
                        <button type="button" 
                                class="flex-1 py-2 text-center transition"
                                :class="absenceModal.form.leave_type === 'unplanned' ? 'bg-orange-100 text-orange-700' : 'bg-gray-50 text-gray-600'"
                                @click="absenceModal.form.leave_type = 'unplanned'; updatePreview()">
                ...
                <div class="form-group">
                    ...
                    <div class="flex border border-gray-300 rounded-md overflow-hidden mb-3">
                        <button type="button" 
                                class="flex-1 py-2 text-center transition"
                                :class="absenceModal.form.coverage_status === 'yes' ? 'bg-green-100 text-green-700 border-r border-green-200' : 'bg-gray-50 text-gray-600'"
                                @click="absenceModal.form.coverage_status = 'yes'">
                            <i class="fas fa-check-circle mr-2"></i>Yes
                        </button>
                        <button type="button" 
                                class="flex-1 py-2 text-center transition"
                                :class="absenceModal.form.coverage_status === 'no' ? 'bg-red-100 text-red-700 border-r border-red-200' : 'bg-gray-50 text-gray-600'"
                                @click="absenceModal.form.coverage_status = 'no'">
                            <i class="fas fa-times-circle mr-2"></i>No
                        </button>
                        <button type="button" 
                                class="flex-1 py-2 text-center transition"
                                :class="absenceModal.form.coverage_status === 'pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-50 text-gray-600'"
                                @click="absenceModal.form.coverage_status = 'pending'">
                            <i class="fas fa-clock mr-2"></i>Pending
                        </button>
                    </div>
                    
                    <!-- If coverage is "Yes", show staff dropdown -->
                    <div v-if="absenceModal.form.coverage_status === 'yes'" class="mt-2">
                    ...
                    <div v-if="absenceModal.form.coverage_status === 'no'" 
